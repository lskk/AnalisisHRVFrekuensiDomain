function net = ecoctrain(net, learner1, X, Y, handles, varargin)
...
C = [C char(10) 'Starting the training'];
        set(handles.edProses,'String',C);
        drawnow;

C = [C char(10) 'This may take a while, since we need to train one SVM per bit ...'];
        set(handles.edProses,'String',C);
        drawnow;
...
for i = 1:net.nbits,
  if net.verbosity>0, 
    info = fprintf('Training for code bit %i (out of %i)\n', i, net.nbits);
    
    C = [C char(10) 'Training for code bit ' num2str(i) ' (out of ' num2str(i) ')'];
        set(handles.edProses,'String',C);
        drawnow;
  end
  ...
  elseif all(Yi0==1),
    ...
    axes(handles.axes1);
    plot(learner{i}.sv),xlabel('row data'),ylabel('support vector');
    legend('ratio LF/HF','nLF','nHF');
  end  
end
net.learner = learner;
